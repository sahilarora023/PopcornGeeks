...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	$sql = "SELECT * FROM users WHERE id='$id' AND username='$u' LIMIT 1";
	...
	$sql = "SELECT * FROM users WHERE id='$id' AND email='$e' LIMIT 1";
    $query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
	// Evaluate for a match in the system (0 = no match, 1 = match)
	if($numrows == 0){
		// Log this potential hack attempt to text file and email details to yourself
		header("location: message.php?msg=Your credentials are missing from our system");
    	exit();
	}
/*	$sql = "SELECT * FROM users WHERE id='$id' AND password='$p' LIMIT 1";
    $query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
	// Evaluate for a match in the system (0 = no match, 1 = match)
	if($numrows == 0){
		// Log this potential hack attempt to text file and email details to yourself
		header("location: message.php?msg=Password chutiya");
    	exit();
	} */
