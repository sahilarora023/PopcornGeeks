...
if (isset($_FILES["avatar"]["name"]) && $_FILES["avatar"]["tmp_name"] != ""){
	...
	if($width < 10 || $height < 10){
		print("<script type='text/javascript'>alert('The image has no dimensions');
	...
	if($fileSize > 1048576) {
		...
		print("<script type='text/javascript'>alert('Please upload a gif, jpg or png image');
		window.location='../user.php';
		</script>");
		exit();
	} else if ($fileErrorMsg == 1) {
		print("<script type='text/javascript'>alert('An error has occured');
		window.location='../user.php';
		</script>");
		exit();
	}
...
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$row = mysqli_fetch_row($query);
	if($row[0] > 14){
		print("<script type='text/javascript'>alert('The website allows only 15 pictures');
